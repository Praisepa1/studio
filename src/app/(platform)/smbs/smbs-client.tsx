...
interface SMBsClientProps {
  initialSMBs: any[];
}

export default function SMBsClient({ initialSMBs }: SMBsClientProps) {
  const [smbs, setSMBs] = useState<any[]>(initialSMBs);
  ...
  const filteredSMBs = smbs.filter((c) => {
    const matchesSearch =
          ...
          <table className="w-full text-sm text-left border-collapse">
            ...
            <tbody className="divide-y">
              {filteredSMBs.map((c) => {
                ...
                const isHiring = c.isActivelyHiring === true;
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div className="space-y-1">
                                ...
                                <p className="text-sm italic text-foreground leading-relaxed">
                                  {c.enrichment?.one_liner || c.enrichment?.description || "No descriptions available yet."}
                                </p>
                              </div>
              ...
              {filteredSMBs.length === 0 && (
                <tr>
                  <td colSpan={7} className="text-center py-12 text-muted-foreground">
                    No SMBs discovered matching current filters.
                  </td>
                </tr>
              )}
            </tbody>
            ...
            return (
              <div key={c.id} className="border rounded-lg p-4 bg-card space-y-3">
                ...
                <p className="text-xs italic text-muted-foreground line-clamp-2">
                  {c.enrichment?.one_liner || c.enrichment?.description || "No description."}
                </p>
